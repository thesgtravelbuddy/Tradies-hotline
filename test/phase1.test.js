...
test('invalid contact data is rejected before storage', async () => {
  ...
  assert.equal(result.status, 422); assert.equal(result.errors.email, 'Email is required'); assert.equal(result.errors.phone, 'Required'); assert.deepEqual(await app.listRequests('dev-business'), []);
});

test('owner can list and open submitted request details', async () => {
  const app = createApp({ repository: new MemoryRequestRepository() }); const created = await app.submitIntake(intake); const requests = await app.listRequests('dev-business'); const detail = await app.getRequest(created.request.id, 'dev-business');
